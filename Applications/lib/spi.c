...
void setupSPI() {
  ...
  GPIOA->MODER |= 1 << GPIO_MODER_MODER15_Pos | // PA11 output (CS)
                  1 << GPIO_MODER_MODER8_Pos;   // PA8  output (CS)
  ...
  GPIOA->BSRR = GPIO_BSRR_BS_15 | GPIO_BSRR_BS_8 ; // set CS
}
...
void beginTransaction() {
  ...
  GPIOA->BSRR = GPIO_BSRR_BR_15; // CS=0
}

void endTransaction() {
  while (SPI1->SR & SPI_SR_BSY)
    ;
  GPIOA->BSRR = GPIO_BSRR_BS_15; // CS=1
}
